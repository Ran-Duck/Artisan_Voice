...
export default function UploadPage() {
  ...
  useEffect(() => {
    if (lastCommand) {
      console.log("[v0] Voice command received:", lastCommand)
  ...
  const startContinuousListening = async () => {
    ...
    console.log("[v0] Starting continuous listening for continue command")

    try {
      while (selectedImage && !isSpeaking) {
        const command = await listen(10000) // Listen for 10 seconds
        if (command) {
          console.log("[v0] Continuous listening received:", command)
          handleVoiceCommand(command)
      ...
      console.error("[v0] Continuous listening error:", error)
    } finally {
