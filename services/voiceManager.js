class VoiceManager {
  ...
  stopAll() {
    console.log("[v0] Stopping all voice activities")
  ...
  async processQueue() {
    ...
    console.log("[v0] Processing voice queue, items:", this.speechQueue.length)

    while (this.speechQueue.length > 0) {
      const { text, options, resolve, reject } = this.speechQueue.shift()

      try {
        console.log("[v0] Speaking from queue:", text.substring(0, 50) + "...")
        await this.directSpeak(text, options)
        resolve()
      } catch (error) {
        console.error("[v0] Queue speech error:", error)
        reject(error)
      }
    }

    this.isProcessingQueue = false
    console.log("[v0] Voice queue processing complete")
  }

  async queueSpeak(text, options = {}) {
    const requestKey = text.substring(0, 50)
    if (this.pendingRequests.has(requestKey)) {
      console.log("[v0] Duplicate request prevented:", requestKey)
      return
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log("[v0] Added to voice queue:", text.substring(0, 50) + "...")
      this.processQueue()
    })
  }
  ...
  async directSpeak(text, options = {}) {
    ...
    if (this.isCurrentlySpeaking) {
      console.log("[v0] Already speaking, ignoring duplicate request")
      return
    }
    ...
    try {
      ...
      console.log("[v0] Making TTS API request for text:", text.substring(0, 50) + "...")
      ...
      console.log("[v0] TTS API request successful")
      ...
      if (error.name === "AbortError") {
        console.log("[v0] TTS request aborted")
        return
      }
  ...
  async playAudio(base64Audio) {
    return new Promise((resolve, reject) => {
      try {
        ...
        console.error("[v0] Audio setup error:", error)
        this.currentAudio = null
        reject(error)
      }
    })
  ...
  fallbackSpeak(text) {
    return new Promise((resolve) => {
      if ("speechSynthesis" in window) {
        ...
        console.log("[v0] Using fallback speech synthesis")
        const utterance = new SpeechSynthesisUtterance(text)
        utterance.onend = () => {
          console.log("[v0] Fallback speech ended")
          resolve()
        }
        utterance.onerror = (error) => {
          console.error("[v0] Fallback speech error:", error)
          resolve()
        }
