...
export function useVoiceLoop() {
  ...
  useEffect(() => {
    return () => {
      console.log("[v0] useVoiceLoop cleanup - stopping all voice activities")
  ...
  const speak = useCallback(async (text: string) => {
    try {
      console.log("[v0] useVoiceLoop speak:", text.substring(0, 50) + "...")
      setIsSpeaking(true)
      setError(null)
      if (voiceManager.isCurrentlySpeaking) {
        console.log("[v0] Already speaking, skipping duplicate request")
        return
      }
      await ttsService.speak(text)
    } catch (err) {
      console.error("[v0] useVoiceLoop speak error:", err)
